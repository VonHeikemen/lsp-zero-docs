...
require('mason').setup({})
require('mason-lspconfig').setup({
  ensure_installed = {'ts_ls', 'rust_analyzer', 'jdtls'},
  handlers = {
...
In `v2.x` each completion item has a label that shows the source that created the item. This feature is now opt-in, you can use the function [.cmp_format()](../reference/lua-api#cmp-format-opts) to get the settings needed for nvim-cmp.
