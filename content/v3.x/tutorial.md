...
There is a <a href="/v4.x/tutorial.html" target="_self">new version</a> of lsp-zero available.
...
If you already have a Neovim configuration setup with a plugin manager, go to the [getting started](./getting-started) page for a quick start.
...
If you are using Neovim v0.7, the branch `compat-07` is compatible with that Neovim version. See the <a href="/compat-07/tutorial.html" target="_self">tutorial in the compat-07 branch</a>.
...
* <a href="/v1.x/tutorial.html" target="_self">Tutorial v1.x</a>
...
lsp-zero has a function that returns a basic config for `lua_ls`, and you can use these settings as a backup. If there isn't any local config file (`.luarc.json`) in the current root directory then you apply the Neovim specific settings to the language server.

Here's the snippet of code that will do that for you:
...
::: details Expand: Manual setup of LSP servers
...
::: details Expand: Automatic setup of LSP servers
