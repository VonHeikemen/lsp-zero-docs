...
Executes the "global" on_attach hook and sets up the local commands. It will also enable the omnifunc for LSP if you used [.omnifunc.setup()](#omnifunc-setup-opts).

This is meant to help you setup a language server with a specialized plugin.

Here is an example using the plugin `flutter-tools`.
...
This is meant to be used with [mason-lspconfig.nvim](https://github.com/williamboman/mason-lspconfig.nvim), in order to help configure automatic setup of language servers. It can be added as a default handler in the setup function of the module `mason-lspconfig`.
...
You can use think of this as an "empty handler" for [mason-lspconfig.nvim](https://github.com/williamboman/mason-lspconfig.nvim). Consider this example.
...
Ideally, you would setup some default values for your servers in your neovim config using [.configure()](#configure-name-opts), or maybe [.store_config()](#store_config-name-opts).
...
This is useful when you want to configure a language using a specialized plugin. See the examples in [quick-recipes](../guide/quick-recipes).
